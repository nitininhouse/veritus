...
const AssetCard: React.FC<AssetCardProps> = ({ asset, vote, verifiers}) => {
  ...
  const getAssetTypeLabel = (assetType: number) => {
    ...
    useEffect(() => {
          if (isPending) {
            ...
            toast.success("Voted successfully.", { id: "claimTxStatus" });
            window.location.reload(); 
          } else if (error) {
            toast.error(`Failed to vote on asset: ${error.message}`, { id: "claimTxStatus" });
    ...
    <div className="relative">
      ...
      <div className="p-6">
        ...
        <div className="mb-5">
          ...
          <Link href={`/assets/${asset.nftTokenId}`} className="text-xl font-bold mb-3 line-clamp-2 group-hover:underline underline-offset-4 transition-all duration-200" style={{ color: theme.textPrimary }}>
          <h3 
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <Link href={`/user/${asset.creator}`} className="hover:underline">
            ...
            {addressExists(verifiers, address?.toString() || '') ? (
              ...
              <>
                <div className="flex items-center gap-1">
                    ...
                    <button 
                      onClick={() => voteOnAsset(BigInt(asset.nftTokenId), true)} 
                      className={`group bg-gradient-to-r ${theme.accent} ${isDarkMode ? 'text-black hover:from-neutral-200 hover:to-neutral-400' : 'text-white hover:from-neutral-800 hover:to-neutral-900'} px-4 py-2 rounded-lg font-semibold transition-all duration-300 transform hover:scale-105 hover:shadow-2xl ${isDarkMode ? 'hover:shadow-white/25' : 'hover:shadow-black/25'} flex items-center gap-2`}
                    >                  
                      Verify                
                    </button>                 

                    <button 
                      onClick={() => voteOnAsset(BigInt(asset.nftTokenId), false)} 
                      className={`group bg-gradient-to-r ${theme.accent} ${isDarkMode ? 'text-black hover:from-neutral-200 hover:to-neutral-400' : 'text-white hover:from-neutral-800 hover:to-neutral-900'} px-4 py-2 rounded-lg font-semibold transition-all duration-300 transform hover:scale-105 hover:shadow-2xl ${isDarkMode ? 'hover:shadow-white/25' : 'hover:shadow-black/25'} flex items-center gap-2`}
                    >                  
                      Report                
                    </button>
        ...
        <Link href={`/assets/${asset.nftTokenId}`}>           
          <button             
            ...
            } px-4 py-2 rounded-lg font-semibold transition-all duration-300 transform hover:scale-105 hover:shadow-2xl ${               
              isDarkMode ? 'hover:shadow-white/25' : 'hover:shadow-black/25'             
            } flex items-center gap-2`}             
            disabled={isPending}           
          >             
            {isPending ? "Processing..." : "View more "}           
          </button>         
        </Link>
      </div>
