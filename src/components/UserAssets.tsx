...
export default function UserAssetsComponent({address}: {address: string}) {
                ...
                {filteredAssets.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {filteredAssets.map((asset) => (
                            <UserAssetCard key={asset.asset.nftTokenId} asset={asset.asset} vote={true} verifiers={asset.verifiers} address={address} />
                        ))}
                    </div>
                )}
            </div>
