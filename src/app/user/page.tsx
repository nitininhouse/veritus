...
export default function UserPage() {
    ...
    const handleRegisterCreator = async () => {
        ...
        try {
            ...
            // Dismiss loading toast when transaction is submitted
    ...
    const handleUpdateProfile = async () => {
        ...
        if (creatorInfo?.wallet == null) {
            handleRegisterCreator();
            return;
        }
        try {
            const loadingToast2 = toast.loading("Please confirm transaction in your wallet...");
            await updateCreatorProfileDetails(formData.name, formData.profileImage, formData.bio);
            toast.dismiss(loadingToast2);
            toast.loading("Confirming transaction...");
            setIsEditing(false);
        } catch (err) {
            toast.error('Updatefailed. Please try again!');
                ...
                <Box sx={{
                    ...
                    <Card sx={{
                        ...
                        <Box sx={{ 
                            ...
                            <UserAssetsComponent address={address?.toString() || ''} />
                        </Box>
                    </Card>
                </Box>
            </Box>
